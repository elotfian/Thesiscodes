...
SL=50;  % Soft limit on archive size
...
for i=1:CL
  i
    Arc(i).pos=Position(xmin,ymin,xmax,ymax,lbc2,nvar);
    Arc(i).cost=MyCost(Arc(i).pos,ndiscp,a,C0,nu,gamRR)';  
end
...
alp=0.9;    %Change in temperature at the end of each iteration
...
%             
       ...
       FF=MyCost(xycheck,ndiscp,a,C0,nu,gamRR)'; 
       break;
       else 
              jj=jj+1;
              if jj==3
