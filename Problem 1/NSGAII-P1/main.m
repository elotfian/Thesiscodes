...
%     id1=0;
%     while id1==0
    pop(i).pos=Position(xmin,ymin,xmax,ymax,lbc2,nvar);
    pop(i).cost=MyCost(pop(i).pos,ndiscp,a,C0,nu,gamRR)';
...
for iter=1:maxiter
    ...
    % crossover
     ...
     crosspop=crossover(crosspop,pop,ncross,nvar,ndiscp,a,C0,nu,gamRR,lbc2);

     mutpop=repmat(emp,nmut,1);
     mutpop=mutation(mutpop,pop,nmut,nvar,ndiscp,a,C0,nu,gamRR,lbc2,xmax,ymax);
...
% figure(2)
% subplot(2,2,1)
% plot(pareto(1).pos(:,1), pareto(1).pos(:,2),'ko','markerfacecolor','k','MarkerSize',4)
% xlim([0 100])
% ylim([0 100])
% %set(gca, 'XTick', [0 20 40 60 80 100],'YTick', [0 20 40 60 80 100])
% set(gcf,'Position',[10 10 900 900])
% subplot(2,2,2)
% plot(pareto(36).pos(:,1), pareto(36).pos(:,2),'ko','markerfacecolor','k','MarkerSize',4)
% xlim([0 100])
% ylim([0 100])
% %set(gca, 'XTick', [0 20 40 60 80 100],'YTick', [0 20 40 60 80 100])
% set(gcf,'Position',[10 10 900 900])
% subplot(2,2,3)
% plot(pareto(21).pos(:,1), pareto(21).pos(:,2),'ko','markerfacecolor','k','MarkerSize',4)
% xlim([0 100])
% ylim([0 100])
% %set(gca, 'XTick', [0 20 40 60 80 100],'YTick', [0 20 40 60 80 100])
% set(gcf,'Position',[10 10 900 900])
% subplot(2,2,4)
% plot(pareto(3).pos(:,1), pareto(3).pos(:,2),'ko','markerfacecolor','k','MarkerSize',4)
% xlim([0 100])
% ylim([0 100])
% %set(gca, 'XTick', [0 20 40 60 80 100],'YTick', [0 20 40 60 80 100])
% set(gcf,'Position',[10 10 900 900])
